...
const Dashboard = () => {
    ...
    const fetchEmergencyStatus = async () => {
        try {
            ...
            console.error('응급실 상태 가져오기 오류:', error);
            message.error('응급실 상태를 가져오는 중 오류가 발생했습니다.');
        }
    };
    ...
    const handleEmergencyStatusToggle = async (status) => {
        try {
            ...
            console.error(error);
            message.error('응급실 상태 변경 중 오류가 발생했습니다.');
        }
    };
    ...
    if (emergencyStatus === null) {
        return (
            <div style={{display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh'}}>
                <Spin tip="데이터를 불러오는 중입니다..." size="large"/>
            </div>
        );
    }
        ...
        <Layout style={{height: '100vh'}}>
            <Header style={{background: '#fff', padding: '20px', display: 'flex', alignItems: 'center'}}>
                <div style={{display: 'flex', alignItems: 'center', gap: '10px', flex: 1}}>
                    ...
                    <Switch
                        checked={emergencyStatus === 'AVAILABLE'}
                        onChange={(checked) => handleEmergencyStatusToggle(checked ? 'AVAILABLE' : 'FULL')}
                        checkedChildren="환자 수용 가능"
                        unCheckedChildren="응급실 FULL"
                        style={{
                            backgroundColor: emergencyStatus === 'AVAILABLE' ? '#52c41a' : '#ff4d4f',
                            color: '#fff',
                            border: 'none',
                        }}
                    />
